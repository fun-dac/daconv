...
class ZoomifyIM:
	...
	IMAGE_EXTENSIONS = ['.tiff', '.TIFF', '.TIF', '.tif', '.png', '.PNG', '.jpg', '.JPG', '.jpeg', '.JPEG']#add extension by takaya
	TMP_EXT = ".tiff"
	TILE_EXT = ".jpg"
	...
	def cutTile(self, rowFile, tilePath, xOffset):
		cmd = "%s \"%s\" -crop %dx%d+%d+0 +repage \"%s\"" % (self.CONVERT, rowFile, self.tileWidth, self.tileHeight, xOffset, tilePath)
		return Run(cmd, getStdout = False)
	#end def cutTile
	...
	def splitFile(self, filePath, x, y):
		...
		cmd = "%s \"%s\" -crop %dx%d +repage \"%s.%%d%s\"" % (self.CONVERT, filePath, x, y, basename, self.TMP_EXT)
